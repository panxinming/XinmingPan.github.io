...
where $$\theta$$ is our estimated parameter.
 ...
 Here, we have 100 outcomes, which are 0 or 1. However, the probability of each outcome is different, and we know from above the probability of $$y=1$$ is $$p(x) = \frac{1}{1+e^{-(\beta_0 + \beta_1 x)}}$$, so
